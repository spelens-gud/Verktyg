...
func StreamLogInterceptor(opts ...func(*LogOption)) grpc.StreamServerInterceptor {
	...
	return func(srv any, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
		...
		start := time.Now()
		lg := logger.FromContext(ss.Context()).WithField("method", info.FullMethod)
		...
		if err != nil {
			fields["error"] = err.Error()
			lg.WithFields(fields).Error("gRPC stream failed")
		} else {
			lg.WithFields(fields).Info("gRPC stream completed")
		}
